...
def main() -> int:
    ...
    (AUDIT / "faithfulness_audit.md").write_text(
        ...
        + rows + "\n\n"
        "## Honest scope\n\n"
        "Operational, not mechanistic: it measures stated-reasoning vs "
        "demonstrated-action consistency, not the model's internal computation. "
        "v0.1 uses rule-based checks on synthetic runs; the semantic "
        "action-outcome check and a counterfactual flip-rate probe (does the "
        "decision change when the cited evidence is invalidated?) are v0.2. The "
        "planted controls only prove the checks fire on the failure types they "
        "encode — not coverage of novel unfaithfulness.\n\n"
